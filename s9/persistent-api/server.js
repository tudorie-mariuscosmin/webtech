...
const CrewMember = sequelize.define('crewMember', {
    ...
    role: {
        type: Sequelize.ENUM,
        allowNull: false,
        values: ['CAPITAN', 'FIRST MATE', 'CREW', 'MECHANIC']
    }
})
...
app.get('/ships', async (req, res, next) => {
    ...
    try {
        const ships = await Ship.findAll(querry)
...
app.get('/ships/:sid/crew-members/:cmid', async (req, res, next) => {
    try {
        ...
        if (ship) {
            ...
            if (crewMember)
                res.status(200).json(ship.crewMember);
        }
        else
            res.status(404).json({ message: 'not found' })
...
app.use((err, req, res, next) => {
    console.warn(err);
    res.status(500).json({ message: "Sserver Erroe" })
})
